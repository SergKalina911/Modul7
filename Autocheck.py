...
Все математические операторы можно переопределить. Для этого существуют методы, отвечающие за каждый оператор:

__add__добавление
__sub__вычитание
__mul__умножение
__div__разделение
__pow__возведение в степень

и другие. Переопределение математических операторов может быть полезным инструментом. Например, давайте создадим 
класс словарей, поддерживающих операции сложения и вычитания:"""
...
""" Синтаксис прост, а код вполне понятен. Однако следует быть осторожным с переопределением математических 
операторов, поскольку такое поведение обычно неочевидно и может сбивать с толку.

7. Реализовать операции сложения и вычитания векторов для Vector класса. То есть переопределить математические 
операторы __add__и __sub__для него.

Имеется два вектора: a с координатами (x1, y1) и b с координатами (x2, y2).

Тогда сложение векторов b + a создаёт новый вектор с координатами (x2 + x1, y2 + y1). Вычитание — обратная операция, 
b - a даёт новый вектор с координатами (x2 - x1, y2 - y1).
...
8. Реализовать операцию скалярного произведения векторов для Vector класса, то есть переопределить математический 
оператор __mul__для неё.

Имеется два вектора: a с координатами (x1, y1) и вектор b с координатами (x2, y2).

Тогда скалярное произведение b*a векторов есть следующее число x2*x1+y2*y1.
...
9. Прежде чем начать сравнивать векторы, необходимо реализовать метод определения длины вектора — len метод для 
Vector класса.

Длина вектора a с координатами (x1, y1) определяется по следующей формуле:
...
""" 10. Необходимо реализовать все методы сравнения для Vector класса. Для простоты будем сравнивать экземпляры 
класса Vector только по длине, используя len метод , не учитывая направление векторов.
...
Протокол итератора в Python реализуется с помощью __iter__ метода . Этот метод должен возвращать итератор. 
Итератором может быть любой объект, имеющий __next__ метод, возвращающий значение при каждом вызове. Для создания 
итератора достаточно реализовать __next__ метод.

Например, давайте создадим Iterable класс и iterator класс:"""
...
""" Обратите внимание, что __next__ метод должен выдать StopIteration исключение, указывающее на завершение 
итерации. В противном случае цикл for для такого объекта будет бесконечным.

Давайте подробнее рассмотрим алгоритм итератора.

Интерпретатор Python принимает контекст итерации и объект, который итерируется в этом контексте.

c = CustomIterator()  # an iterable object created
for i in c:  # an iteration context (`for` loop) and an object to be iterated in it, an instance of class `c`
    print(i)

Правила интерпретатора в такой ситуации указывают, что для итерируемого объекта c получить итератор означает 
вызвать его __iter__ метод, возвращающий Iterable() объект-итератор. После этого следует вызвать __next__ метод 
Iterable() объекта-итератора. __next__ Метод возвращает нечто, что передаётся в переменную цикла (в нашем случае
— последовательность чисел от 1 до 10).

На следующем шаге итерации (в цикле for) для существующего объекта итератора снова вызывается Iterable() его
__next__ метод (как и на предыдущем шаге), а результат вызова также присваивается переменной цикла i и передается 
в работу тела цикла.

Метод __next__ отслеживает количество возможных вызовов самого себя и при достижении лимита, который в нашем 
случае определяется параметром MAX_VALUE, генерирует StopIteration исключение. Это сигнал интерпретатору о 
