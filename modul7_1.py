...
У мові програмування Python, поля - це змінні, які зберігають інформацію про стан об'єкта. Доступ до цих полів та 
їх модифікація зазвичай відбувається безпосередньо, але іноді необхідно контролювати цей процес, наприклад, для 
валідації даних або інкапсуляції. Для цього існують спеціальні методи які називають гетерами та сетерами. В Python 
...
інкапсуляції. Наприклад, якщо потрібно завжди повертати значення поля у вигляді рядка, навіть якщо воно 
зберігається як число.

Сетери (від англ. set - встановлювати) - це методи, які дозволяють встановити значення поля. Вони найчастіше 
використовуються для валідації даних, які намагаються присвоїти полю. Наприклад, якщо ми маємо поле, який повинно 
приймати значення лише додатні числа, можна в сетері додати перевірку, яка буде викидати виняток або повертати 
помилку, якщо намагатися присвоїти йому від'ємне число.

Вбудований декоратор @property в Python дозволяє легко створювати гетери. Використання цього декоратора робить 
метод класу доступним як поле, тобто його можна буде викликати без дужок. Це робить інтерфейс класу більш чистим 
та інтуїтивно зрозумілим. Для створення сетера для того ж поля, що і гетер, використовується декоратор 
@property.setter, який застосовується до методу з тим же ім'ям, що і властивість.

Розглянемо приклад класу, який використовує @property для створення гетера та сетера для поля age. Ми хочемо, щоб 
age завжди був додатнім числом. Спочатку ми створимо метод age з декоратором @property, який буде виступати як 
гетер і повертати значення атрибута __age (приватна версія age, що використовується внутрішньо класом для 
зберігання реального значення). Потім ми створимо метод age з декоратором @property.setter, який буде виступати як 
сетер і валідувати вхідне значення перед його присвоєнням __age. Якщо значення не відповідає нашим критеріям 
(наприклад, якщо воно від'ємне), ми можемо викинути виняток або виконати інші дії для обробки помилки.

Почнемо з оголошення класу Person, який має приватний атрибут __age. Згадайте, що ми використовуємо підкреслення 
на початку імені атрибута, щоб позначити, що цей атрибут є внутрішнім і не повинен бути доступний для прямого 
доступу ззовні класу. Це стандартна практика для інкапсуляції, яка допомагає зберегти внутрішню реалізацію класу 
...
if __name__ == "__main__":
    ...
    print(person.age)
    try:
        person.age = -5  # Спроба встановити від'ємне значення
    except ValueError as e:
        print(e)
...
""" У цьому прикладі, коли ми створюємо новий екземпляр класу Person, конструктор __init__ приймає вік як аргумент 
і присвоює його приватному атрибуту __age. Гетер для age просто повертає значення цього приватного атрибуту, 
дозволяючи зовнішньому коду отримувати вік особи без прямого доступу до внутрішньої реалізації. 

@property
def age(self):
    return self.__age  # Геттер повертає значення приватного поля

Сетер age виконує валідацію вхідного значення перед його присвоєнням приватному полю __age. Якщо вказане значення 
є від'ємним, сетер генерує виняток ValueError, запобігаючи тим самим некоректному присвоєнню віку. Це забезпечує, 
що об'єкт Person завжди має валідний вік.
...
Використання гетерів та сетерів з декоратором @property дозволяє нам забезпечити контрольований доступ до полів 
класу, виконуючи необхідні перевірки або обробку при спробі читання або зміни їх значень. Це підвищує безпеку та 
надійність коду, дозволяючи розробникам визначати як атрибути мають бути використані, та які дії повинні бути 
виконані при їх зміні.
...
Це відбувається тому, що в конструкторі ми напряму присвоюємо значення атрибуту __age, минаючи сетер.
Виправимо цей недолік. Необхідно в конструкторі класу спочатку значення __age встановити в None, а потім встановити 
в передане значення age через сетер. Це дозволить нам одразу застосувати логіку валідації, визначену в сетері, при 
...
# Виведення:
# ValueError: Вік не може бути від'ємним
""" У цьому варіанті, коли ми створюємо екземпляр класу Person, спочатку приватному полю __age присвоюється 
значення None. Потім викликається сетер age, передаючи йому значення, передане у конструктор. Це дозволяє нам 
використовувати логіку валідації, визначену в сетері, вже на етапі ініціалізації об'єкта, забезпечуючи, що 
некоректні дані не будуть призначені атрибуту __age.

Такий підхід підвищує надійність і безпеку коду, оскільки він гарантує, що всі значення, які присвоюються важливому 
атрибуту об'єкта age, проходять через однаковий процес валідації незалежно від того, чи вони встановлені при 
створенні об'єкта, чи змінюються пізніше.
...
# False
# True
""" Використовуючи декоратор @property, ми створюємо гетери та сетери для захищеного та приватного поля. Це 
дозволяє нам контролювати доступ до цих атрибутів, забезпечуючи можливість валідації даних та інкапсуляції 
внутрішньої реалізації. 


                        Статичні та класові методи

Статичні та класові методи є спеціальними типами методів, які можуть використовуватися в класах для різноманітних 
цілей. Вони схожі тим, що обидва викликаються без створення екземпляру класу, але мають різну мету та 
використовуються в різних сценаріях.

Статичні методи використовують декоратор @staticmethod і є методами, які не мають доступу до екземпляру класу тобто 
змінної self, з якого вони були викликані. Це означає, що статичні методи не можуть змінювати стан об'єкта або 
класу, але вони можуть бути корисними для виконання деяких операцій, які не залежать від стану об'єкта. Статичні 
методи можна розглядати як "допоміжні" функції, які мають логічний зв'язок із класом, але не потребують доступу до 
його атрибутів або методів.

Наприклад, якщо у нас є клас, який представляє геометричні фігури, ми можемо включити статичний метод для 
обчислення площі кола, використовуючи радіус, який передається як аргумент. Цей метод не залежить від конкретного 
екземпляру класу та може бути викликаний без створення об'єкта."""
class Geometry:
    PI = 3.14159
...
"""
У цьому прикладі area_of_circle є статичним методом, який обчислює площу кола, використовуючи переданий радіус. Цей 
метод може бути викликаний безпосередньо з класу без необхідності створення екземпляру класу.

Щоб використати цей метод ми викликаємо area_of_circle безпосередньо через клас Geometry, передаючи радіус як 
аргумент. Метод використовує цей радіус для обчислення та повертає площу кола.

Статичний метод не залежить від якогось конкретного екземпляру Geometry і може бути викликаний без створення такого 
екземпляру.

Класові методи використовують декоратор @classmethod і, на відміну від статичних методів, мають доступ до самого 
класу через параметр cls, який автоматично передається Python. Це означає, що класові методи можуть змінювати стан 
класу або викликати інші класові методи. Класові методи часто використовуються для фабричних методів, які створюють 
екземпляри класу, використовуючи різні способи ініціалізації, ніж стандартний конструктор.

Наприклад, якщо ми маємо клас Employee, ми можемо використати класовий метод для створення екземплярів класу на 
основі інформації, отриманої з рядка або файлу."""
class Employee:
...
"""
В нашому коді, from_string є класовим методом, який дозволяє створювати екземпляри Employee, розбираючи рядок на 
поля. Метод використовує параметр cls для створення нового екземпляра, гарантуючи, що він може бути успішно 
використаний навіть у класах-нащадках. В нашому прикладі cls це буде сам клас Employee."""
...
"""
Ми викликаємо from_string, передаючи рядок з інформацією про співробітника. Метод розбирає рядок, використовуючи 
кому як роздільник, і створює новий екземпляр класу Employee, використовуючи отримані дані. Потім ми можемо 
використовувати цей екземпляр, як звичайний, звертаючись до його атрибутів name та position.

Різниця між статичними та класовими методами полягає в тому, що статичні методи використовуються для функцій, які 
не потребують доступу до атрибутів або методів класу, тоді як класові методи мають доступ до класу та його 
атрибутів, дозволяючи змінювати стан класу або створювати екземпляри класу за допомогою альтернативних конструкторів.
"""